...
from ..validation import Template
...
class FlowLinks(ScopePropertyNode):
    ...
    __LINK_TEMPLATE__ = {"From": __LINK_CONFIG_TEMPLATE__,
        ...
        try:
            for direction in ["From", "To"]:
                ...
                node_inst, _, port_name, _, port_type = re.findall(
                    FlowLinks.LINK_REGEX, link[direction])[0]

                output[direction] = {"node_inst": node_inst, "port_name": port_name,
                                     "port_type": port_type, "str": link[direction]}

            dep_level = link.get("Dependency", 0)
            output["Dependency"] = dep_level if (
                3 >= dep_level >= 0) else 0
    ...
    def get_link(self, key: str) -> Template:
        ...
        """
        return Template.load_dict(FlowLinks._parse_link(self.value[key]), FlowLinks.__LINK_TEMPLATE__)
    ...
    def add(self, source_node: str, source_port: str, target_node: str, target_port: str,
            source_type: str = "", target_type: str = "") -> tuple:
        """
        Add a new link
        """
