...
class Callback(Scope):
    ...
    def get_methods(module: str) -> dict:
        try:
            ...
            methods = {'modules': list(modpkgs_names),
    ...
    def template_depends(self, force=False):
        ...
        for node_key in full_node_list:
            ...
            if force:
                node['Node'][node_name]['PortsInst'][node_inst_name]['In'][iport_name]['Callback'] = 'null_callback'
                MovaiDB().set(node)
                return 'True'
        ...
        for elem in replaced_cbs:
            try:
                dependencies[elem[0]].append(
                    {'NodeInst': elem[1], 'Port': elem[2]})
            except:
