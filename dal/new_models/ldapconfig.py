...
class LdapConfig(MovaiBaseModel):
    ...
    PrimaryHost: str
    PrimaryPort: int
    SecondaryHost: Optional[str]
    SecondaryPort: Optional[int]
    ...
    @classmethod
    def create(cls, ldap_parameters: dict) -> MovaiBaseModel:
        ...
        try:
            config = cls(
                name=ldap_parameters['DomainName'],
                DomainName = ldap_parameters['DomainName'],
                SecretKey = generate_secret_string(LDAP_KEY_LENGTH),
                PrimaryHost = ldap_parameters.get('PrimaryHost'),
                PrimaryPort = ldap_parameters.get('PrimaryPort'),
                SecondaryHost = ldap_parameters.get('SecondaryHost'),
                SecondaryPort = ldap_parameters.get('SecondaryPort'),
                SSLVersion = ldap_parameters.get('SSLVersion'),
                Username = ldap_parameters.get('Username'),
                Password = ldap_parameters.get('Password'),
                UsersDN = ldap_parameters.get('UsersDN'),
                GroupsDN = ldap_parameters.get('GroupsDN'),
            )
            
            config.update_validation(False)
            config.save()
            LOGGER.info(f"creating LdapConfig entry named {config.name}")
            return config
        except ValueError:
            error_msg = f"The Ldap configuration named {config.name}"\
                    " is already exist."
    ...
    def remove(cls, config_name: str) -> None:
        ...
        config.delete()
        LOGGER.info(f"{cls.__name__}:{config_name} have been removed")
    ...
    def list_config_names(cls) -> list:
        ...
        LOGGER.debug(f"current LdapConfig entries: {configs}")
        return configs
