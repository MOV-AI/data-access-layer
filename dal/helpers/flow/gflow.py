    ...
    def generate_remaps(self) -> dict:
        ...
        while ports:
            ...
            for link in port["links"]:
                ...
                port_to_remap = (
                    links[link]["To"] if port["_type"] == "From" else links[link]["From"]
                )
                ...
                if self.graph[port_to_remap]["remap"] is not None:
                    ...
                    if (
                        ...
                        if link_in_forced_remap:
                            ...
                            forced_link = self.check_forced_links(
                                port["_type"], self.flow.Links[link]
                            )
                            if forced_link:
                                ...
                                port_of_unremapable_if_exists = self.get_adjacent_unremappable_port(
                                    port["_type"], port["links"]
                                )
