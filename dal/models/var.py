...
class Var:
    ...
    def __setattr__(self, name, value, expire: float = None):
        ...
        MovaiDB(scope).set({'Var':{self.scope:{'ID':{prefix+name:{'Value': value}}}}}, px=milis)

    def __getattr__(self, name):
        scope, prefix = self.__get_scope()
        try:
            return MovaiDB(scope).get_value({'Var':{self.scope:{'ID':{prefix+name:{'Value': ''}}}}}, search=False)
        except KeyError:
            return None
    ...
    def __get_scope(self):
        ...
        prefixes = {'callback': self._node_name + '@' + self._port_name + '@',
                    'node': self._node_name + '@', 'robot': '@', 'flow': 'flow@',
                    'fleet': self._robot_name + '@', 'global': '@'}
    ...
    def delete_all(scope: str = 'Node', _robot_name='', _node_name='', _port_name=''):
        ...
        prefixes = {'callback': _node_name + '@' + _port_name + '@',
                    'node': _node_name + '@', 'robot':'@', 'flow':'flow@',
        ...
        if scope == 'node': #also clean SM Vars
            MovaiDB(scope_).unsafe_delete({'Var':{scope:{'ID':{prefix+'*':{'Parameter': '*'}}}}})
