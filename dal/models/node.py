...
class Node(Scope):
    ...
    def get_params(self, attribute='Parameter'):
        ...
        node_name = self.name
        params = MovaiDB().get(
            {'Node': {node_name: {attribute: {'*': {'Value': ''}}}}})
    ...
    def remove(self, force=False) -> bool:
        ...
        for port_inst_name in self.PortsInst.keys():
            ...
            has_port_inst_dependencies = True if not has_port_inst_dependencies and port_node_instance_links else False
        ...
        try:
            ...
            if force:
                ...
                for node_inst_ref_key in node_inst_ref_keys:
                    ...
                    node_inst_name = next(iter(node_inst_ref_key.get(
                        'Flow').get(flow_name).get('NodeInst')))
    ...
    def rename(self, key, old_name, new_name):
        ...
        if key == 'PortsInst':
            ...
            new_depends = eval(str(old_depends).replace(
                '/'+old_name+'/', '/'+new_name+'/'))
    ...
    def template_depends(self, force=False):  # TODO use Links hash
        ...
        if not deleted_nodes and not deleted_links:
            super().remove()
            return {'event': 'delete', 'data': {'numberDependencies': 0, 'dependencies': {}}}
        ...
        for elem in list(set(deleted_links)):
            from_ = MovaiDB().search(
                {'Flow': {elem[0]: {'Link': {elem[1]: {'From': '*'}}}}})[0].rsplit(':', 1)[1]
            to_ = MovaiDB().search(
                {'Flow': {elem[0]: {'Link': {elem[1]: {'To': '*'}}}}})[0].rsplit(':', 1)[1]
            try:
    ...
    def get_port_node_instance_links(self, port_name: str) -> list:
        ...
        for node_key in full_node_list:
            try:
                ...
                flow_links = MovaiDB().get(
                    {'Flow': {flow_name: {'Links': '*'}}})
                node_inst_name = next(
                    iter(node.get('Flow').get(flow_name).get('NodeInst')))

                for key, value in flow_links.get('Flow', {}).get(flow_name, {}).get('Links', {}).items():

                    if re.search(r'^{0}/{1}/.+$'.format(node_inst_name, port_name), value.get('From')):
                        dict_key = {
                            'Flow': {flow_name: {'Links': {key: value}}}}
                        node_ref_keys.append(dict_key)

                    if re.search(r'^{0}/{1}/.+$'.format(node_inst_name, port_name), value.get('To')):
                        dict_key = {
                            'Flow': {flow_name: {'Links': {key: value}}}}
                        node_ref_keys.append(dict_key)
    ...
    def port_inst_depends(self, port_name: str) -> list:
        ...
        flow_exposed_ports = MovaiDB().get(
            {'Flow': {'*': {'ExposedPorts': '*'}}})
        ...
        # Join all keys
        reference_keys = [*node_ref_keys, *
                          flow_container_link_keys, *exposed_ports_ref_keys]
    ...
    def get_exposed_port_node_instance_links(self, port_name, exposed_port_node_inst_name):
        ...
        # Loop through Flows with Containers instances and get Links to the port that is exposed
        flow_container_link_keys = []
        flows_with_containers = MovaiDB().get(
            {'Flow': {'*': {'Container': '*'}}})
        ...
        for (flow_name, flows_containers) in flows_with_containers.get('Flow').items():
            for container_node_inst_name in flows_containers.get('Container').keys():
                ...
                container_flow_name = flows_containers\
                    .get('Container', {})\
                ...
                node_inst_name = "{}__{}".format(
                    prefix, exposed_port_node_inst_name)
