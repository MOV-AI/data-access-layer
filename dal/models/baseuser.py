...
class BaseUser(Model):
    ...
    def create(cls,
        ...
        if cls.is_exist(domain_name, account_name):
            error_msg = f"The requested user {account_name}@{domain_name} "\
                        "already exist"
        ...
        user.write()
        msg = f"Successfully created new {cls.__name__} named:"\
              f"{principal_name}"
        cls.log.info(msg)
        return user
    ...
    def update(self, user_params: dict):
        ...
        if not any(key in user_params.keys() for key in self.update_keys):
            error_msg = f"Json fields aren't found in "\
    ...
    def email(self, address: str) -> None:
        ...
        if len(address) > self.max_attr_length:
            raise ValueError(f"The address agrument must be less "
                             f"than {self.max_attr_length}")
        self.Email = address
    ...
    def roles(self, roles: list) -> None:
        ...
        if len(roles) > self.max_attr_length and len(roles) > min_roles_count:
            error_msg = f"The roles agrument must be greater than"\
    ...
    def list_users(cls, domain_name: str) -> list:
        ...
        cls.log.debug(f"current list of BaseUser records found in the "
                     f"system: {users_names}")
        return users_names
    ...
    def has_permission(self,
                       resource_name: str,
                       permission_name: str,
                       skip_superuser: bool = False) -> bool:
